...
// larkChatID: 可选，用于存储飞书的 chat_id，以便回复时找到原始接收方
func (c *Client) SendMessage(text string, channel string, username string, files []FileInfo, extraData map[string]interface{}) error {
	...
	// 合并额外数据
	if len(files) > 0 || len(extraData) > 0 {
	...
	// 调试：打印发送的 JSON
	c.logger.Debug("发送消息到 Matterbridge", "json", string(jsonData))
...
func (c *Client) StartWebSocket(ctx context.Context) error {
	for {
		...
		default:
			...
			// 调试：打印收到的消息
			c.logger.Debug("收到 Matterbridge 消息", "json", string(data))
